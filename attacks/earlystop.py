...
def Early_PGD(model, data, target, step_size, epsilon, perturb_steps, tau,loss_fn, rand_init=True, omega=0, input_hw = 32):
    '''
    ...
    while K > 0:
        ...
        if len(output_index) != 0:
            if len(output_target) == 0:
                ...
                output_adv = iter_adv[output_index].reshape(-1, 3, input_hw, input_hw).cuda()
                output_natural = iter_clean_data[output_index].reshape(-1, 3, input_hw, input_hw).cuda()
                output_target = iter_target[output_index].reshape(-1).cuda()
            else:
                # incorrect adv data should not keep iterated
                output_adv = torch.cat((output_adv, iter_adv[output_index].reshape(-1, 3, input_hw, input_hw).cuda()), dim=0)
                output_natural = torch.cat((output_natural, iter_clean_data[output_index].reshape(-1, 3, input_hw, input_hw).cuda()), dim=0)
    ...
    if len(output_target) == 0:
        ...
        output_adv = iter_adv.reshape(-1, 3, input_hw, input_hw).cuda()
        output_natural = iter_clean_data.reshape(-1, 3, input_hw, input_hw).cuda()
    else:
        output_adv = torch.cat((output_adv, iter_adv.reshape(-1, 3, input_hw, input_hw)), dim=0).cuda()
        output_target = torch.cat((output_target, iter_target.reshape(-1)), dim=0).squeeze().cuda()
        output_natural = torch.cat((output_natural, iter_clean_data.reshape(-1, 3, input_hw, input_hw).cuda()),dim=0).cuda()
    output_adv = output_adv.detach()
    return output_adv, output_target, output_natural, count
