...
class TinyImageNetLoader:
    def __init__(self, root_path, num_labels=-1, num_class=200):
        self.root_path = os.path.join(root_path, 'tiny-imagenet-200')
    ...
    def get_lb_train_data(self):
        ...
        data = TinyImagenetDataset(root=os.path.join(self.root_path, "train"), transform=transform, ulb=False,
                                   num_labels=self.num_labels)
        return data

    def get_ulb_train_data(self):
        transform = self.get_transform(train=True, ulb=True)
        data = TinyImagenetDataset(root=os.path.join(self.root_path, "train"), transform=transform, ulb=True)
        return data

    def get_lb_test_data(self):
        transform = self.get_transform(train=False, ulb=False)
        data = TinyImagenetDataset(root=os.path.join(self.root_path, "val"), transform=transform, ulb=False)
        return data
...
class SSL_Dataset:
    ...
    def __init__(self,
        ...
        crop_size = 96 if self.name.upper() == 'STL10' else 64 if self.name.upper() == 'TINY-IMAGENET' else 224 if self.name.upper() == 'IMAGENET' else 32
        #self.transform = get_transform(mean[name], std[name], crop_size, train)
        self.transform = get_transform(crop_size, train=False if self.name == 'svhn' else self.train)

    def get_data(self, svhn_extra=False):
        """
        get_data returns data (images) and targets (labels)
        shape of data: B, H, W, C
        shape of labels: B,
        """
        if self.name not in ['tiny-imagenet']:
        ...
        if 'CIFAR' in self.name.upper():
        ...
        elif self.name.upper() == 'TINY-IMAGENET':
            if self.train:
                dset = ImageFolder(os.path.join(self.data_dir, 'train'))
                data_path = dset.imgs
            
            #data = [Image.open(path) for _, path, _  in enumerate(data_path)]
            ...
            #data
                ...
                dset = ImageFolder(os.path.join(self.data_dir, 'val'))
                data_path = dset.imgs
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        ulb_dset = BasicDataset(self.alg, ulb_data, ulb_targets, self.num_classes,
                                self.transform, True, None, onehot)
        #ulb_dset = BasicDataset(self.alg, ulb_data, ulb_targets, self.num_classes,
        #                        self.transform, is_ulb = False, strong_transform=None, onehot=False)
